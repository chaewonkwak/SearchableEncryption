...
/*
prf_type Utilities::convertToArray(std::string addr) {
    prf_type res;
    for (int i = 0; i < 16; i++) {
        res[i] = addr[i];
    }
...
*/
prf_type Utilities::encode(std::string keyword) {
    unsigned char plaintext[16];
    for (unsigned int i = 0; i < keyword.length(); i++) {
        plaintext[i] = keyword.at(i);
    }
    for (int i = keyword.length(); i < 16 - 4; i++) {
        plaintext[i] = '\0';
    }
...
prf_type Utilities::encode(std::string keyword, unsigned char* curkey) {
    unsigned char plaintext[16];
    for (unsigned int i = 0; i < keyword.length(); i++) {
        plaintext[i] = keyword.at(i);
    }
    for (int i = keyword.length(); i < 16; i++) {
        plaintext[i] = '\0';
    }
    if (curkey == NULL) {
        curkey = key;
...
prf_type Utilities::encode(unsigned char* plaintext, unsigned char* curkey) {
    if (curkey == NULL) {
        curkey = key;
    }
    unsigned char ciphertext[16];
...
std::string Utilities::decode(prf_type ciphertext, unsigned char* curkey) {
    unsigned char plaintext[16];
    ...
    std::string result;
    for (int i = 0; i < 16 && plaintext[i] != '\0'; i++) {
        result += (char) plaintext[i];
    }
    return result;
}

void Utilities::decode(prf_type ciphertext, prf_type& plaintext, unsigned char* curkey) {
    ...
    memcpy(plaintext.data(), plain, 16);
}
